...
This module builds the redesigned Spine Proofreading control panel.
The layout and interactions are kept independent from the legacy widget tree,
while the section navigation can be populated from real morphology data.

Usage (from a notebook, with ``lib`` on ``sys.path``):

    import proofreading_interface
    proofreading_interface.display_preview(mesh_path, morphology_path)
"""
...
class DesignState:
    ...
    def __init__(self, mesh_path=None, morphology_path=None):
        ...
        if self.morphology_path is None:
            ...
            self.neuron_id = loaded['neuron_id']
...
class SpineValidationDesign:
    ...
    def __init__(self, mesh_path=None, morphology_path=None):
        self.state = DesignState(mesh_path=mesh_path, morphology_path=morphology_path)
        self._silent = False
    ...
    def _layout(self):
        ...
        root = widgets.VBox(
            [self.header_html, body, viewport],
    ...
    def _on_subsection_done(self, _button):
        ...
        section['missing_status'] = (
            'Missing'
            if sum(sub['missing_count'] for sub in section['subsections']) > 0
            else 'No Missing'
        )
        if self.state.subsection_index < len(section['subsections']) - 1:
            self._set_status('Subsection marked done.')
            self._select_subsection(self.state.subsection_index + 1)
            return

        # Match the legacy workflow: completing the final subsection returns
        # the reviewer to the first spine in the section.
        self.state.spine_index = 0
...
def build_preview(mesh_path=None, morphology_path=None, fonts_dir=None):
...
def display_preview(mesh_path, morphology_path, fonts_dir=None, show_sample_viewport=False):
